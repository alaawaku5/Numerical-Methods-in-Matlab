...
% Script code for testing (EXERCISE 8 PART)
a = @(x) 1;
f = @(x) ones(size(x));
n = 100;
t = linspace(0, 1, n+1);
...
g = @(x) 0.5 .* x .* (1 - x);
figure;
plot(t, xSol, 'DisplayName', 'FEM solution'); 
hold on;
plot(t, g(t), 'DisplayName', 'Exact solution');
grid on;
xlabel('x');
ylabel('u(x)');
...
%%
% ode for problem 11 (tracking computational time and sparse usage)
% we will measure cpu times for n=2^k and compare full and sparse approach

% define the range n=2^k
kVals = 5:20; 
nVals = 2.^kVals;
...
figure;
loglog(nVals, timeFull, 'DisplayName','full');
hold on;
loglog(nVals, timeSparse, 'DisplayName','sparse');
grid on;
xlabel('n (number of elements)');
ylabel('time (sec)');
title('full and sparse matrices');
